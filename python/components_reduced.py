'''
-----------------------------------------------
title: components_reduced.py
description: A collection of algorithms used to drive a low power, offline camera surveillance system. These algorithms
			aim to aid the obtaining of a stream, perform motion detection of such a stream, perform background
			filtering processes on saved images, and also manage the available storage.
author: AF Grobler
for: Project (E) 448, Department of Electric and Electronic Engineering, University of Stellenbosch
-----------------------------------------------

TODO: Write an additional algorithm that periodically checks the available storage, and then randomly deletes a number of
	  images in the storage folder to free up storage if necessary.
'''
...
class MotionDetectorLFR:
	...
	NOTE: Sometimes, due to the way the stream is now handled and buffered in a different thread, combined with the forced lower frame rate, 
	the processor and source of the stream become out of sync to a point where the stream can no longer be read.

	TODO: Refresh the stream in regular intervals to ensure that the source and processor stay in sync.
	"""
	frame = 0
...
class MotionDetector2:
	...
	The idea of this experimental class is to generate an activity heatmap from 5000 frames every [interval] minutes, and then compare the position
	of detected motion to the heatmap, to only save images where motion was detected in 'cold' zones.

	TODO: Implement a forced lower frame rate.
	"""
	hm_list = []
	last_check_time = 0
...
class SimilarityDetector:
	'''
	The SimilarityDetector class is used to mainly loop over a directory of images and compare the each image only the the one occurring ater it.
	An algorithm that compares each image to all other images was also considered, but is too computationally taxing and simply takes too long to execute.

	In addition, the files in the directory are sorted according to date modified. This is to ensure that the images are analysed in chronological order.
	Images that are too similar get moved to a storage directory, and the ultimate goal of this class is to ensure that no two consecutive images are too similar.
	'''
	...
	def determine_similarity(self, prev_frame, current_frame):
		...
		'''
		Uses a mixture of histogram comparison and template matching to calculate a % similarity between two images.
		'''
...
class CameraManager:
	'''
	Houses all functional components used in the GUI file.
	'''
